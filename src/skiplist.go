...
func NewSkipList() *SkipList {
	arena := newArena(uint32(nodeSize) * 2000)
	//arena := newArena(1)
...
func (s *SkipList) findSpliceForLevel(key []byte, befOffset uint64, level int) (bef uint64, after uint64) {
	for {
		befNode := s.arena.getNode(befOffset)
		nxtOffset := atomic.LoadUint64(&befNode.tower[level])
...
func (s *SkipList) getNextNode(n *Node, level int) *Node {
	nxtOffset := atomic.LoadUint64(&n.tower[level])
...
func (s *SkipList) Add(e *Entry) {
	...
	var prev [maxHeight + 1]uint64
	var nxt [maxHeight + 1]uint64
	...
	for level := int(listHeight) - 1; level >= 0; level-- {
		...
		if prev[level] == nxt[level] {
			l, valOffset := s.arena.putVal(v)
			encValue := encodedValue(valOffset, l, v.EncodedSize())
	...
	//if !s.verify() {
	//	res, equal := s.findNear(key, true, true)
	//	origin := s.arena.getNode(nodeOffset)
	//	//0x14000400000
	//	fmt.Printf("%p\n", &s.arena.buf[0])
	//	fmt.Println(newNode, res.getKey(s.arena), equal, origin.getKey(s.arena))
	//	print("123")
	//}
}
